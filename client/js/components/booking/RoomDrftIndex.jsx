...
export default class RoomDrftIndex extends React.Component {
  ...
  renderLocationRoomCards() {
    return this.props.rooms.map((room) => {
      if (room.node.resources.length) {
        return <div key={room.node.name}><h3>{room.node.name}</h3><RoomCards loading={this.props.loading} rooms={this.displayableRooms(room.node.resources)} routeParams={this.routeParams(room.node)} query={this.props.query} /></div>
      }
    })
  }
  ...
  renderLocationAvailabilities() {
    return this.props.rooms.map((room) => {
      if (room.node.resources.length) {
        return <div key={room.node.name}><h3>{room.node.name}</h3><AvailabilityMatrix rooms={room.node.resources} routeParams={this.props.routeParams} query={this.props.query}></AvailabilityMatrix></div>
      }
    })
  }
