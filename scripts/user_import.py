...
def import_users(data_file, reject_file, save=False):
    ...
    for u in user_data:
        ...
        if user:
            matches = matches + 1
            logger.debug("Match (%d) on username %s = %s" % (matches, username, user.get_full_name()))
            ...
            if user:
                matches = matches + 1
                logger.debug("Match (%d) on email %s = %s" % (matches, email, user.get_full_name()))
                reject_file.write(json.dumps(u))
            else:
    ...
    reject_file.write("]")
    reject_file.close()
    logger.debug("Users: %d, Matches: %d" % (users, matches))
