...
class Location(models.Model):
	...
	stay_page = models.TextField(default="This is the page which has some descriptive text at the top (this text), and then lists the available rooms and beds. HTML is supported.")
	...
	def rooms_with_future_reservability(self):
		...
		for room in Room.objects.filter(location=self):
			if room.has_future_reservability():
				future_reservability.append(room)
		return future_reservability

	def _beds_with_future_reservability_queryset(self):
		# used in forms.py
		today = timezone.localtime(timezone.now())
		return Bed.objects.filter(reservables__isnull=False).filter(room__location=self).filter(Q(reservables__end_date__gte=today) | Q(reservables__end_date=None)).distinct()

	def availability(self, start, end):
		# show availability (num free beds), between start and end dates.
		# create a structure available_beds[room] with an ordered list of
		# date, beds free pairs {the_date: XX, beds_free: YY}. so it can be
		# iterated over in order of date, in the template. 
		...
		for room in rooms_at_location:
			...
			available_beds[room] = []
			while the_day < end:
				if not room.is_reservable_this_day(the_day):
					available_beds[room].append({'the_date': the_day, 'beds_free' : 0})
				else:
					beds_this_room = 0
					for bed in room.beds.all():
						if bed.is_free_this_day(the_day):
							beds_this_room += 1
...
class Room(models.Model):
	...
	image = models.ImageField(upload_to=room_img_upload_to)

	def __unicode__(self):
		return self.name

	def get_free_beds_on_dates_or_none(self, arrive, depart):
		beds = self.beds.all()
		free = []
		for b in beds:
			if b.is_free_between(arrive, depart):
				free.append(b)
		return free

	def has_future_reservability(self):
		today = timezone.localtime(timezone.now())
		beds = self.beds.all()
		for bed in beds:
			if bed.has_future_reservability():
				return True
		return False
	...
	def is_reservable_this_day(self, this_day):
		...
		beds = self.beds.all()
		for bed in beds:
			if bed.is_reservable_this_day(this_day):
				return True
		return False
...
class Bed(BookableResource):
	''' assumption is that beds are bookable by day. in the future we may have
	other types of resources bookable in different time increments, like
	hourly.'''
	room = models.ForeignKey(Room, related_name='beds')

	def __unicode__(self):
		return "%s bed (bed id %d)" % (self.room.name, self.id)
	...
	def is_free_this_day(self, this_day):
	...
	def is_free_between(self, arrive, depart):
		# must a. be reservable, and b. not have any other reservations. 
		day = arrive
		while day < depart:
			if self.is_free_this_day(day):
...
class Reservation(models.Model):
	...
	def generate_bill(self, delete_old_items=True, save=True, reset_suppressed=False):
		...
		room_charge_desc = "%s (%d * $%s)" % (self.bed.room.name, self.total_nights(), self.get_rate())
	...
	def reset_rate(self):
		self.set_rate(self.bed.default_rate)
...
class PaymentManager(models.Manager):
	...
	def reservation_payments_by_bed(self, bed):
		reservation_payments = Payment.objects.filter(bill__in=ReservationBill.objects.filter(reservation__bed=bed))
		return reservation_payments
