...
class Migration(DataMigration):
	...
	def forwards(self, orm):
		# Move the rate over to the reservation
		for r in orm['core.Reservation'].objects.all():
			for p in orm['core.Payment'].objects.filter(reservation = r):
				r.rate = p.rate
			if not r.rate:
				r.rate = 0
			r.save()
		...
		for p in orm['core.Payment'].objects.all():
